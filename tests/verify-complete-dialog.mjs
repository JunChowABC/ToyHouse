...
const manifest = JSON.parse(await readFile(new URL("../assets/level-complete-popup2-v1/manifest.json", import.meta.url), "utf8"));
for (const asset of Object.values(manifest.assets)) {
  const bytes = await readFile(new URL(`../${manifest.directory}/${asset.file}`, import.meta.url));
  assert.deepEqual(bytes, await readFile(new URL(`../docs/${manifest.directory}/${asset.file}`, import.meta.url)));
  const source = asset.source_path || (asset.source_layer ? `art/level-complete-popup2/${asset.file}` : `outputs/level-complete-popup2-layered-v1/layers/${asset.file}`);
  assert.deepEqual(bytes, await readFile(new URL(`../${source}`, import.meta.url)), asset.file + " matches approved source");
}
  ...
  for (const [entry, width, height, dpr] of [["/", 540, 960, 1], ["/docs/", 390, 844, 2], ["/docs/", 1024, 768, 1]]) {
    ...
    await page.addInitScript(() => {
      ...
      proto.drawImage = function (image, ...args) {
        if (image.src?.includes("/level-complete-popup2-v1/")) trace.images.push({ file: image.src.split("/").pop(), bounds: args, scale: this.getTransform().a });
    ...
    assert.equal(state.completion.art.loaded, 45);
    assert.equal(state.completion.art.version, "level-complete-popup2-v1");
    ...
    const card = trace.images.find(image => image.file === "ui_coin_card.png");
    const icon = trace.images.find(image => image.file === "ui_reward_coin.png");
    assert.equal(trace.images.length, 42, "render separate art; only one reward card/count plate/icon triplet");
    assert.equal(card.bounds[0] + card.bounds[2] / 2, 941 / 2);
    assert.ok(Math.abs(icon.bounds[0] + icon.bounds[2] / 2 - 941 / 2) <= 1);
    assert.ok(!trace.images.some(image => /ui_gem_card|ui_gem_count_plate|ui_reward_gem/.test(image.file)));
    for (const value of ["×30", "返回首页", "下一关"]) assert.ok(trace.texts.some(text => text.value === value), value);
    for (const id of ["arttext_level_complete", "arttext_rewards", "art_celebration", ...manifest.staticLayers]) {
      assert.deepEqual(trace.images.find(image => image.file === `${id}.png`)?.bounds, manifest.assets[id].bounds, id + " source placement");
    }
    assert.ok(!trace.texts.some(text => ["×100", "×3", "×50", "×2", "关卡完成", "获得奖励"].includes(text.value)), "titles use PSD art, amounts use actual reward");
    const count = trace.texts.find(text => text.value === "×30");
    assert.ok(Math.abs(count.bounds[0] - 941 / 2) < 1);
    await shot("single-coin");
    // Both button art and attached decorations press together; dragging away cancels.
    for (const key of ["home", "next"]) {
      await page.waitForTimeout(180);
      const r=state.uiHitAreas.completion[key],box=await page.locator("#game").boundingBox();
      ...
      const held=await page.evaluate(() => {window.advanceTime(0);return window.__completeTrace;});
      for (const id of manifest.buttonLayers[key]) {
        assert.ok(Math.abs(held.images.find(i=>i.file===`${id}.png`).scale/trace.images.find(i=>i.file===`${id}.png`).scale-.92)<.001,id+" pressed with button");
      }
      await page.mouse.move(box.x+2,box.y+2);await page.mouse.up();
      assert.equal((await read()).mode,"level-complete");
      assert.equal((await read()).economy.coins,30);
    }
    await advance(60000);
    ...
    const cards = dual.images.filter(image => /^ui_(coin|gem)_card\.png$/.test(image.file));
    ...
    const starCard = star.images.filter(image => /^ui_(coin|gem)_card\.png$/.test(image.file));
    ...
    assert.ok(!empty.images.some(image => /^ui_(coin|gem)_card\.png$/.test(image.file)));
