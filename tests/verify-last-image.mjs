...
try{for(const entry of ['/', '/docs/']){
 ...
 await page.route('**/home-v2/bg_room.png*',r=>{if(block)held.push(r);else return r.continue();});
 ...
 assert.equal((await read()).homeArt.loaded,88);
 assert.equal((await read()).art.ready,true);
 assert.ok((await read()).art.loading.pending.some(p=>p.endsWith('/bg_room.png')));
 ...
 await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).homeArt.loaded===89);
