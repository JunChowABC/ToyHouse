...
const out = new URL("../test-output/v5-art/", import.meta.url);
await mkdir(out, { recursive: true });
const manifest = JSON.parse(await readFile(new URL("../assets/toyhouse-v5/manifest.json", import.meta.url), "utf8"));
...
try {
  for (const [entry, width, height, dpr] of [["/", 540, 960, 1], ["/docs/", 390, 844, 2], ["/docs/", 1024, 768, 1]]) {
    ...
    await page.addInitScript(() => {
      const trace = window.__artDrawTrace = { rabbits: [], texts: [], rugs: [], roundRects: 0 };
      const p = CanvasRenderingContext2D.prototype;
      const clear = p.clearRect, draw = p.drawImage, text = p.fillText, round = p.roundRect;
      p.clearRect = function (...args) {
        trace.rabbits = []; trace.texts = []; trace.rugs = []; trace.roundRects = 0;
    ...
    assert.equal(state.art.rabbitPose, "ears-follow-direction");
    assert.equal(state.levelTitle, "月光敲敲窗");
    assert.ok(Math.abs(state.art.rug.w / state.art.rug.h - 933 / 955) < 1e-6);
    ...
    assert.ok(trace.texts.includes("第01关·月光敲敲窗"));
    ...
    for (const id of Object.keys(manifest.assets)) {
      const bytes = await readFile(new URL(`../assets/toyhouse-v5/${manifest.assets[id].file}`, import.meta.url));
      const release = await readFile(new URL(`../docs/assets/toyhouse-v5/${manifest.assets[id].file}`, import.meta.url));
      assert.deepEqual(bytes, release);
    }
    const remove = state.uiHitAreas.tools.find(t => t.id === "remove");
    await page.evaluate(() => window.__toyhouse_debug.grantReward({ id: "v5-art-fixture", source: "task", tools: { remove: 1, flip: 1, shuffle: 1 } }));
  ...
  console.log("v5 assets: source/release parity, mobile/tablet layouts, tool hit targets, pause and combo passed");
} finally {
  await browser.close();
}
